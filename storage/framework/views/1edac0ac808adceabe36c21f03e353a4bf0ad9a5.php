			...
			<div class="panel panel-inverse">
				...
				<div class="panel-body">
					<table id="tbl_business_lst" class="table table-striped table-bordered">
						...
						<tbody>
							<?php $__currentLoopData = $businessNotApproved; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							<tr class="gradeC" id="<?php echo e($row->BUSINESS_ID); ?>">
								<td><?php echo e($row->BUSINESS_OR_NUMBER); ?></td>
								<td><?php echo e($row->BUSINESS_NAME); ?> <br> ( <?php echo e($row->LINE_OF_BUSINESS_NAME); ?>)</td>
								<td><?php echo e($row->BUSINESS_ADDRESS); ?></td>
								<td><?php echo e($row->BUSINESS_OWNER_LASTNAME); ?>, <?php echo e($row->BUSINESS_OWNER_FIRSTNAME); ?>, <?php echo e($row->BUSINESS_OWNER_MIDDLENAME); ?></td>
								
								<?php if($row->NEW_RENEW_STATUS == "New"): ?>
			...
			<div class="panel panel-inverse">
				...
				<div class="panel-body">
					<table id="tbl_pending_issuance" class="table table-striped table-bordered">
						...
						<tbody>
							<?php $__currentLoopData = $pending_application_form; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
							<tr class="gradeC" id="<?php echo e($row->BUSINESS_ID); ?>">
								<td><?php echo e($row->BUSINESS_OR_NUMBER); ?></td> 
								...
								<td hidden><?php echo e($row->BUSINESS_NATURE_NAME); ?></td> 
  ...
  <div class="input-group-append">
    <span class="input-group-text">.00</span>
							...
							<div class="form-group row m-b-10">
								<label class="col-sm-3 col-form-label">Approved By</label>
								<div class="col-sm-9">
									<input type="text" class="form-control" placeholder="" id="txt_evaluate_by_issuance" value="<?php echo e(session('session_full_name')); ?>" readonly>
								</div>
							</div>
...
<script>
	...
	$('#btnEvaluateIssuance').on('click', function(){
		...
		$.ajax({
			url : "<?php echo e(route('IssuanceEvaluation')); ?>",
			method : 'POST',
			data : data,
	...
	$('#modal-Approval').on('click', '#btnEvaluate', function(){
		...
		$.ajax({
			url : "<?php echo e(route('CRUDBusinessApproval')); ?>",
			method : 'POST',
			data : data,
			success : function(response) {
				swal({
...
<script src="<?php echo e(asset('custom/momentjs/moment.min.js')); ?>"></script>

<?php $__env->stopSection(); ?>

<?php echo $__env->make('global.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\BITBO_INSTALLER\4. BITBo\www\resources\views/permit_certification_clearance/verification.blade.php ENDPATH**/ ?>